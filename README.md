...
Ship your work end to end. A Claude Code plugin: a configurable orchestrator that drives a batch of issues (or your current local changes) through implement, review, comment cleanup, and PR, keeps living docs in sync. The same steps are also standalone, individually callable skills, so you can run just the CI fix, just the review, or push the current diff through the flow without a tracker.
...
Early scaffolding. First skill in place: `comment-cleanup`. The generalized engine (work-units, sources, stage skills, doc jobs, the ship-issues orchestrator and `init`) is being built out, extracted and generalized from a proven per-project implementation.
